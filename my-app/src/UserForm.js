...
function UserForm(){
   ...
   // };
    return(
        ...
        <select value={selectedOption} ocChange={(e)=> setSelectedOption(e.target.value)}>
            <option value="">select on option</option>
            <option value="option1">option1</option>
            <option value="option2">option2</option>
            <option value="option3">option3</option>
        </select>
        <input type="radio" id="option1" name="options" value="option1" checked={selectedRadio==='option1'} onChange={(e)=>setSelectedRadio(e.target.value)}/>option1
        <input type="radio" id="option2" name="options" value="option2" checked={selectedRadio==='option2'} onChange={(e)=>setSelectedRadio(e.target.value)}/>option2
        <input type="radio" id="option3" name="options" value="option3" checked={selectedRadio==='option3'} onChange={(e)=>setSelectedRadio(e.target.value)}/>option3
        <input type="checkbox" id="subscribe" name="subscribe" value="subscribe" checked={selectedCheckbox} onChange={(e)=>setSelectedCheckbox(e.target.checked)}/>
        {name}
        <br/>
         {selectedCheckbox ? "subscribe":"not subscribe"}<br/>
        </>
    )
}
    function SingleForm(){
        const [form,setForm]=React.useState({name:'',selectedOption:'',selectedRadio:'',selectedCheckbox:false});
    ...
    function validateForm(){
        const newErrors={};// name,selectedOption:please select an option
        if(!form.name){
